...
export const SearchAndWindows = () => {
    ...
    const entryIconRevealer = Widget.Revealer({
        ...
        hpack: "end",
        child: Widget.Label({
            className: "icon-material overview-search-icon",
            label: "search"
        })
    });
    ...
    const entry = Widget.Entry({
        ...
        onChange: (entry) => {
            ...
            // Add fallback: search
            if (userOptions.search.enableFeatures.aiSearch) resultsBox.add(AiButton({ text: entry.text }));
            if (userOptions.search.enableFeatures.webSearch) resultsBox.add(SearchButton({ text: entry.text }));
