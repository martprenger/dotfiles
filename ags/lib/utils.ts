...
const display = Gdk.Display.get_default() || new Gdk.Display();
...
export function toggleWindow(windowName: string) {
  ...
  const isTransparent = transparentScrimWindowNames.get().includes(windowName);
  ...
  if (window.visible) {
    ...
    if (isTransparent) {
      ...
      activePopupWindows("opaque").forEach((win) => (win.visible = false));
...
export function monitorDashboard() {
  monitorFile(`${GLib.getenv("HOME")}/.config/ags/style/dashboard.scss`, () => {
    const target = "/tmp/astal/style.css";
    exec(`sass ${GLib.getenv("HOME")}/.config/ags/style/main.scss ${target}`);
    App.apply_css(target);
  });
