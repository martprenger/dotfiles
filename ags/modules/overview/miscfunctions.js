...
export function launchCustomCommand(command) {
    ...
    if (args[0] == ">raw") {
        // Mouse raw input
        Utils.execAsync("hyprctl -j getoption input:accel_profile").then((output) => {
        ...
        execAsync([
            ...
            .catch(print)
            .then(
                execAsync(["bash", "-c", `${App.configDir}/scripts/color_generation/switchwall.sh --noswitch`]).catch(
                    print
                )
            )
            .catch(print);
    } else if (args[0] == ">pywal") {
        // Use Pywal (ik it looks shit but I'm not removing)
        execAsync([
            `bash`,
            `-c`,
